...
is absent, preparation uses each Runtime Binding home's valid `models_cache.json`.
For a missing cache, it exports the corresponding CLI's complete built-in catalog
with `debug models --bundled` in a private home without inherited credentials.
The existing model metadata comparison still applies. A present invalid cache,
unsupported or failed export, or missing selected model blocks promotion.
Bundled snapshots and their binary identities publish with the existing transaction;
collection does not write user caches. An invalid explicit catalog does not fall
back to default data. Generated overlays retain their custom or default origin,
and default sources are selected afresh on repeat updates. Feature inventories
